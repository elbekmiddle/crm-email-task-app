...
**Mitigation:**
- `companyId` is *never* accepted from the client. `GET /tasks` and
  `POST /tasks/:id/review` both derive it server-side from `TenantGuard`
  (resolved from the bearer token), and every repository query filters by
  `{ ..., companyId }` — including the single-task lookup for review, so a
  guessed/enumerated task ID from another tenant simply returns 404, not the
  task.
...
**Known gap:** the token is per-company, not per-user, so there's no
"employee A vs employee B within the same company" boundary or audit trail
of *who* reviewed a task.
...
**Mitigation:**
- The system prompt constrains the model to a fixed JSON schema and nothing
  else (`response_format: json_object`), and the email content is passed as
  data in the user message, not concatenated into the system prompt.
